...
void GeometryCreator::create(Primitive* primitive, const std::vector<glm::vec4>& pos, const std::vector<glm::vec4>& norm, const std::vector<glm::vec2>& uv, const std::vector<glm::ivec3>& faces)
{
	...
	geo.pos_buf = (std::unique_ptr<TextureBuffer>)(new TextureBuffer(sizeof(glm::vec4) * primitive->num_pos, GL_RGBA32F));
	...
	geo.normal_buf = (std::unique_ptr<TextureBuffer>)(new TextureBuffer(sizeof(glm::vec4) * primitive->num_pos, GL_RGBA32F));
	geo.normal_buf->upload(norm.data());

	primitive->cpu_norm = std::unique_ptr<std::vector<glm::vec4>>(new std::vector<glm::vec4>(primitive->num_pos));
	memcpy(primitive->cpu_norm->data(), norm.data(), geo.normal_buf->m_size);

	primitive->uv_buf = (std::unique_ptr<TextureBuffer>)(new TextureBuffer(sizeof(glm::vec2) * primitive->num_pos, GL_RG32F));
	primitive->uv_buf->upload(uv.data());

	primitive->cpu_uv = std::unique_ptr<std::vector<glm::vec2>>(new std::vector<glm::vec2>(primitive->num_pos));
	memcpy(primitive->cpu_uv->data(), uv.data(), primitive->uv_buf->m_size);
